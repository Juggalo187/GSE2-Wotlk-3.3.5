        ...
        if GSE.isEmpty(string.find(instring, '--', 1, true)) then
            for cmd, etc in string.gmatch(instring or '', '/(%w+)%s+([^\n]+)') do
                ...
                elseif Statics.CastCmds[string.lower(cmd)] then
                    ...
                    local foundspell, returnval = GSE.TranslateSpell(etc, mode, (cleanNewLines and cleanNewLines or false), absolute)
                    if foundspell then
        ...
        if mode == "CURRENT" then
            if GSEOptions.showCurrentSpells then
                ...
                if test then
					...
					local _, _, _, _, _, _, currentSpell = GetSpellInfo(etc)
