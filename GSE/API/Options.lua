      ...
      generalTab = {
        ...
        args = {
          ...
          editorHeight = {
            ...
            desc = L["How many pixels high should the Editor start at."],
            type = "input",
            set = function(info,val) if tonumber(val) >= (GetScreenHeight()) then GSEOptions.editorHeight = tonumber(val) end end,
            get = function(info) return tostring(GSEOptions.editorHeight) end,
            order = 204
          },
          editorWidth = {
            name = L["Default Editor Width"],
            desc = L["How many pixels wide should the Editor start at."],
            type = "input",
            set = function(info,val) if tonumber(val) >= (GetScreenWidth()) then GSEOptions.editorWidth = tonumber(val) end end,
      ...
      macroResetTab = {
        ...
        args = {
          ...
          clearCommonKeyBinds = {
            ...
            desc = L["This function will remove the SHIFT+N, ALT+N and CTRL+N keybindings for this character.  Useful if [mod:shift] etc conditions don't work in game."],
            type = "execute",
