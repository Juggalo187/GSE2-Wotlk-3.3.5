...
function GSE:ZONE_CHANGED_NEW_AREA()
    --local name, type, difficulty, difficultyName, maxPlayers, playerDifficulty, isDynamicInstance, mapID,
		...
		local name, intype, difficulty, difficultyName, maxPlayers, playerDifficulty, isDynamicInstance = GetInstanceInfo()
    if intype == "pvp" then
        GSE.PVPFlag = true
    else
        GSE.PVPFlag = false
    ...
    if intype == "arena" then
        GSE.inArena = true
    else
        GSE.inArena = false
    end
    if intype == "scenario" or difficulty == 167 or difficulty == 152 then
        GSE.inScenario = true
    else
