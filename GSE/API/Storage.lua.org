...
function GSE.OOCUpdateSequence(name, sequence)
    ...
    if pcall(GSE.CheckSequence, sequence) then
        ...
        -- Only translate a sequence if the option to use the translator is on
        tempseq = GSE.TranslateSequence(tempseq, name, "STRING", true)
        tempseq = GSE.UnEscapeSequence(tempseq)
        
        -- Build execution sequence for 3.3.5
        local executionseq = {}
        ...
        -- Store for icon updates
        GSE.SequencesExec[name] = executionseq

        -- SIMPLIFIED APPROACH FOR 3.3.5 - Use basic macro text
        -- For 3.3.5, we'll use the first step only or combine steps
        local macroText = ""
        if #executionseq > 0 then
            macroText = executionseq[1] -- Use first step for simplicity
        end
        
        gsebutton:SetAttribute("macrotext", macroText)
        gsebutton:SetAttribute("step", 1)
        gsebutton:SetAttribute("clicks", 1)
        gsebutton:SetAttribute("limit", 1)
        ...
        -- Simple key press/release for 3.3.5
        ...
        -- Use simplified OnClick for 3.3.5
        gsebutton:WrapScript(gsebutton, 'OnClick', GSE.PrepareSimpleOnClick(sequence))
...
function GSE.CreateMacroString(macroname)
    -- Use a simpler approach for 3.3.5
...
function GSE.PrepareOnClickImplementation(sequence)
    ...
    -- Ensure proper modifier context for secure execution
    returnstring = [=[
local mods = {}
mods.alt = IsAltKeyDown()
mods.ctrl = IsControlKeyDown()
mods.shift = IsShiftKeyDown()
]=] .. returnstring .. format(Statics.OnClick, GSE.PrepareStepFunction(sequence.StepFunction, GSE.IsLoopSequence(sequence)))
