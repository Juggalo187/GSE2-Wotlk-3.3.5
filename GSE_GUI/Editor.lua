...
editframe.Left, editframe.Bottom, editframe.Width, editframe.Height = editframe.frame:GetBoundsRect()
    ...
    if editframe.Height < (GetScreenHeight() / 1.5)  then
        editframe.Height = (GetScreenHeight() / 1.5)
        editframe:SetHeight(editframe.Height)
    end
    if editframe.Width < (GetScreenWidth() / 2) then
        editframe.Width = (GetScreenWidth() / 2)
...
function GSE.GUIEditorPerformLayout(frame)
    ...
    nameeditbox:SetText(editframe.SequenceName or "")
...
function GSE:GUIDrawMetadataEditor(container)
    ...
    scrollcontainer:SetHeight(editframe.Height - 330)
    ...
    -- Classic WoW (Wrath of the Lich King 3.3.5)
    ...
    defgroup4:AddChild(arenadropdown)
...
-- The rest of the file remains the same (GUIDrawMacroEditor and other functions)
-- Only removing the irrelevant content from the metadata editor above
...
function GSE:GUIDrawMacroEditor(container, version)
    ...
    layoutcontainer:SetHeight(editframe.Height - 370)
    layoutcontainer:SetLayout("Flow")

    local scrollcontainer = AceGUI:Create("SimpleGroup")
    scrollcontainer:SetWidth(editframe.Width)
    scrollcontainer:SetHeight(editframe.Height - 370)
    scrollcontainer:SetLayout("Fill")
    ...
    local smallbox = 2
    local largebox = 8
    if editframe.Height > (GetScreenHeight() / 2) then
        local framesize = editframe.Height / (GetScreenHeight() / 2)
