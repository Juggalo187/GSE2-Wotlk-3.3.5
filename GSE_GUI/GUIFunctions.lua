...
function GSE.GUIDeleteSequence(classid, sequenceName)
  ...
  local dialog = StaticPopup_Show("GSE-DeleteMacroDialog")
  if dialog then
    dialog:SetFrameStrata("FULLSCREEN_DIALOG")
    dialog:Raise() -- Ensure it's brought to front
  end
end

--- This function then deletes the macro.
function GSE.GUIConfirmDeleteSequence(classid, sequenceName)
  -- Hide UI frames first
  ...
  -- Clear from ActiveSequences if it exists there
  if GSE.ActiveSequences and GSE.ActiveSequences[tonumber(classid)] then
    GSE.ActiveSequences[tonumber(classid)][sequenceName] = nil
  end
  
  -- Clear editor references if this was the edited sequence
  if GSE.GUIEditFrame and GSE.GUIEditFrame.SequenceName == sequenceName then
    GSE.GUIEditFrame.SequenceName = ""
    GSE.GUIEditFrame.Sequence = {}
    GSE.GUIEditFrame.save = false
    GSE.GUIEditFrame.NewSequence = false
  end
  
  -- Clear from any cached button references
  if GSE.UsedSequences and GSE.UsedSequences[sequenceName] then
    GSE.UsedSequences[sequenceName] = nil
  end
  
  -- Clear from execution sequences
  if GSE.SequencesExec and GSE.SequencesExec[sequenceName] then
    GSE.SequencesExec[sequenceName] = nil
  end
  
  -- Also check and clear from global storage (class 0) if it exists there
  if GSE.Library[0] and GSE.Library[0][sequenceName] then
    GSE.Library[0][sequenceName] = nil
  end
  if GSEStorage[0] and GSEStorage[0][sequenceName] then
    GSEStorage[0][sequenceName] = nil
  end
  
  -- Force a reload to refresh all internal state
  GSE.PerformReloadSequences()
  
  GSE.Print(string.format(L["Sequence '%s' has been completely deleted."], sequenceName))
