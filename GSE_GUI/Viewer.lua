...
-- });
DEFAULT_CHAT_FRAME:AddMessage("Width = "..GetScreenWidth().."Height = "..GetScreenHeight())
GSE.GUIViewFrame = viewframe

--viewframe.Height = 500
--viewframe.Width = 700

viewframe.Height = GetScreenHeight() / 2
viewframe.Width = GetScreenWidth() / 2
...
function GSE.GUICreateSequencePanels(frame, container, key)
  ...
  selpanel:SetCallback("OnClick", function(widget, _, selected, button)
	...
	local elements = GSE.split(widget:GetKey(), ",")
		if viewframe.SequenceName ~= tostring(elements[2]) then
